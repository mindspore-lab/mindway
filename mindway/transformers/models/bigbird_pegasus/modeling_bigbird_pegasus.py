...
class BigBirdPegasusLearnedPositionalEmbedding(mint.nn.Embedding):
    """
...
class BigBirdPegasusScaledWordEmbedding(mint.nn.Embedding):
    """
...
class BigBirdPegasusEncoderAttention(nn.Cell):
    def __init__(self, config, seed=None):
        ...
        self.output = mint.nn.Linear(config.hidden_size, config.hidden_size, bias=config.use_bias)
...
BIGBIRD_PEGASUS_INPUTS_DOCSTRING = r"""
    Args:
        ...
        decoder_head_mask (`ms.Tensor` of shape `(num_layers, num_heads)`, *optional*):
...
class BigBirdPegasusEncoder(BigBirdPegasusPreTrainedModel):
    ...
    def __init__(self, config: BigBirdPegasusConfig, embed_tokens: Optional[mint.nn.Embedding] = None):
    ...
    def _pad_to_block_size(self, hidden_states: ms.Tensor, attention_mask: ms.Tensor):
