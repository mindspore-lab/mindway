...
def router_z_loss_func(router_logits: ms.Tensor) -> float:
    r"""
    ...
    log_z = mint.logsumexp(router_logits, dim=-1)
    z_loss = log_z**2
    return mint.sum(z_loss) / (num_groups * tokens_per_group)


def load_balancing_loss_func(router_probs: ms.Tensor, expert_indices: ms.Tensor) -> float:
    r"""
    ...
    Args:
        router_probs (`ms.Tensor`):
            Probability assigned to each expert per token. Shape: [batch_size, seqeunce_length, num_experts].
        expert_indices (`ms.Tensor`):
    ...
    expert_mask = mint.nn.functional.one_hot(expert_indices, num_experts)

    # For a given token, determine if it was routed to a given expert.
    expert_mask = mint.max(expert_mask, dim=-2)[0]

    # cast to float32 otherwise mean will fail
    expert_mask = expert_mask.to(ms.float32)
    tokens_per_group_and_expert = mint.mean(expert_mask, dim=-2)

    router_prob_per_group_and_expert = mint.mean(router_probs, dim=-2)
    return mint.mean(tokens_per_group_and_expert * router_prob_per_group_and_expert) * (num_experts**2)
...
class SwitchTransformersTop1Router(nn.Cell):
    ...
    def __init__(self, config: SwitchTransformersConfig):
        ...
        self.classifier = mint.nn.Linear(config.hidden_size, self.num_experts, bias=config.router_bias)
        self.jitter_noise = config.router_jitter_noise
        self.ignore_padding_tokens = config.router_ignore_padding_tokens
        self.dtype = getattr(ms, config.router_dtype)

    def _compute_router_probabilities(self, hidden_states: ms.Tensor) -> Tuple[ms.Tensor, ms.Tensor]:
        r"""
        ...
        Returns:
            router_probabilities (`ms.Tensor`):
        ...
        router_probabilities = mint.nn.functional.softmax(router_logits, dim=-1, dtype=self.dtype).to(self.input_dtype)
    ...
    def construct(self, hidden_states: ms.Tensor) -> Tuple:
        r"""
        ...
        Returns:
            Tuple[`ms.Tensor`, `ms.Tensor`, `ms.Tensor`] Tuple containing the expert index, the router probs
            and the router logits. The router probabilities and logits are required to compute the loss.
        """
        router_probs, router_logits = self._compute_router_probabilities(hidden_states)

        expert_index = mint.argmax(router_probs, dim=-1)
        expert_index = mint.nn.functional.one_hot(expert_index, num_classes=self.num_experts)

        # Mask tokens outside expert capacity. Sum over each sequence
        token_priority = mint.cumsum(expert_index, dim=-2)
        # mask if the token routed to to the expert will overflow
        expert_capacity_mask = token_priority <= self.expert_capacity
        expert_index = expert_index * expert_capacity_mask

        router_probs = mint.max(router_probs, dim=-1)[0].unsqueeze(-1)
...
class SwitchTransformersSparseMLP(nn.Cell):
    ...
    def construct(self, hidden_states):
        ...
        expert_index = mint.argmax(router_mask, dim=-1)
        ...
        idx_mask = mint.nonzero(idx_mask, as_tuple=True)[
            0
...
class SwitchTransformersBlock(nn.Cell):
    ...
    def construct(
        ...
        if isinstance(hidden_states, tuple):
            ...
            router_tuple = (mint.zeros((1,), dtype=ms.int64),)
...
class SwitchTransformersPreTrainedModel(MSPreTrainedModel):
    """
...
class SwitchTransformersModel(SwitchTransformersPreTrainedModel):
    ...
    def construct(
        ...
        >>> from mindway import SwitchTransformersModel
        ...
        >>> # This is not needed for mindspore's SwitchTransformersForConditionalGeneration as it does this internally using labels arg.
...
class SwitchTransformersForConditionalGeneration(SwitchTransformersPreTrainedModel, GenerationMixin):
    ...
    def construct(
        ...
        >>> from mindspore import tensor
        >>> from transformers import AutoTokenizer
        >>> from mindway import SwitchTransformersForConditionalGeneration
        >>> tokenizer = AutoTokenizer.from_pretrained("google/switch-base-8")
        ...
        >>> # inference
        >>> input_ids = tokenizer(
        ...     "summarize: studies have shown that owning a dog is good for you", return_tensors="np"
        ... ).input_ids  # Batch size 1
        >>> outputs = model.generate(tensor(input_ids))
        ...
        if output_router_logits:
            ...
            if self.encoder.config.encoder_sparse_step > 1:
                ...
                encoder_router_probs = mint.nn.Softmax(dim=-1)(encoder_router_logits)
            ...
            if self.decoder.config.decoder_sparse_step > 1:
                ...
                decoder_router_probs = mint.nn.Softmax(dim=-1)(decoder_router_logits)
        ...
        if labels is not None:
            loss_fct = mint.nn.CrossEntropyLoss(ignore_index=-100)
    ...
    def _unpack_router_logits(self, router_outputs):
        ...
        return mint.cat(total_router_logits, dim=1), mint.cat(total_expert_indexes, dim=1)
...
class SwitchTransformersEncoderModel(SwitchTransformersPreTrainedModel):
    ...
    def construct(
        ...
        output_router_logits: Optional[bool] = True,
        return_dict: Optional[bool] = None,
    ) -> Union[Tuple[ms.Tensor], MoEModelOutput]:
        r"""
        Returns:
        ...
        >>> from mindspore import tensor
        >>> from transformers import AutoTokenizer
        >>> from mindway import SwitchTransformersEncoderModel
        >>> tokenizer = AutoTokenizer.from_pretrained("google/switch-base-8")
