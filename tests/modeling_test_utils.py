...
def compute_diffs(pt_outputs: Union[torch.Tensor, Tuple, List, BaseModelOutput], ms_outputs: ms.Tensor):
    ...
    for p, m in zip(pt_outputs, ms_outputs):
        if isinstance(p, BaseModelOutput):
            p = tuple(p.last_hidden_state)[0]
        if isinstance(p, tuple):
            for index, value in enumerate(p):
                p = p[index] if isinstance(p[index], np.ndarray) else p.detach().cpu().numpy()
                m = m[index] if isinstance(m[index], np.ndarray) else m.asnumpy()
